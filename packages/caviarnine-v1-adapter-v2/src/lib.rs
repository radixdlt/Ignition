...
//! This is a v2 of the Caviarnine adapter moves the bin number from being a
//! constant defined in code to being stored in the state. This is done to allow
//! for it to be different for different pools.
...
macro_rules! define_error {
    (
        ...
        $(
            const $name: &'static str = concat!("[Caviarnine v1 Adapter v2]", " ", $item);
        )*
    };
}
...
define_error! {
    ...
    POOL_HAS_NO_PREFERRED_NUMBER_OF_BINS
        => "The provided pool does not have a preferred number of total bins";
}
...
#[types(ComponentAddress, PoolInformation, Decimal, PreciseDecimal, u32)]
pub mod adapter {
    enable_method_auth! {
        roles {
    ...
    struct CaviarnineV1Adapter {
        ...
        /// A map that stores the preferred total number of the higher and lower
        /// bins to use for each liquidity pool. If no entry is found in this
        /// map then an error will be thrown.
        preferred_total_number_of_higher_and_lower_bins:
            KeyValueStore<ComponentAddress, u32>,
    }
    ...
    impl CaviarnineV1Adapter {
        ...
        pub fn upsert_preferred_total_number_of_higher_and_lower_bins(
            &mut self,
            pool: ComponentAddress,
            bins: u32,
        ) {
            self.preferred_total_number_of_higher_and_lower_bins
                .insert(pool, bins)
        }
        ...
        fn open_liquidity_position(
            ...
            let SelectedTicks {
                ...
                *self
                    .preferred_total_number_of_higher_and_lower_bins
                    .get(&pool_address)
                    .expect(POOL_HAS_NO_PREFERRED_NUMBER_OF_BINS),
            );
